...
for data, label in [(month1, "Month 1"), (month2, "Month 2"), (month3, "Month 3")]:
    report = Report(metrics=[ColumnDriftMetric(column_name=feature_to_track)])
    report.run(reference_data=reference, current_data=data)
    result = report.as_dict()

    metric = result["metrics"][0]["result"]
    drift_score = float(metric["drift_score"])
    threshold = metric["stattest_threshold"]
